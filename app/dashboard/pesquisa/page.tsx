    ...
    <div style={{ padding: '0', minHeight: '100vh' }}>
      <div style={{ background: '#0d0d0d', borderBottom: '1px solid #1a1a1a', padding: '18px 28px' }}>
        <h1 style={{ margin: 0, fontSize: '16px', fontWeight: '600' }}>
          <span style={{ color: '#C9A84C', marginRight: '8px' }}>◎</span>Pesquisa Jurídica
        </h1>
        <p style={{ margin: 0, fontSize: '12px', color: '#555' }}>Jurisprudência STF, STJ, TST, TJs em linguagem natural</p>
      </div>
      ...
      <div style={{ padding: '32px 28px' }}>
        ...
        <form onSubmit={search} style={{ marginBottom: '32px' }}>
          <div style={{ position: 'relative', maxWidth: '800px', margin: '0 auto' }}>
            <div style={{ position: 'absolute', left: '16px', top: '50%', transform: 'translateY(-50%)', fontSize: '18px', color: '#555' }}>◎</div>
            <input
              value={query}
              onChange={e => setQuery(e.target.value)}
              placeholder='Ex: "validade cláusula não-concorrência prazo máximo"'
              style={{ paddingLeft: '44px', paddingRight: '120px', height: '52px', fontSize: '14px', borderRadius: '10px', border: '1px solid #2a2a2a' }}
            />
          ...
          <div style={{ display: 'flex', gap: '8px', justifyContent: 'center', marginTop: '12px', flexWrap: 'wrap' }}>
            {["STF", "STJ", "TST", "TJ-SP", "TRF", "Todas as cortes"].map(t => (
              <button key={t} type="button" style={{
                padding: '4px 12px', borderRadius: '4px', border: '1px solid #2a2a2a',
        ...
        {loading && (
          <div style={{ textAlign: 'center', padding: '60px 0' }}>
            ...
            <div style={{ color: '#C9A84C', fontWeight: '600', marginBottom: '6px' }}>Pesquisando jurisprudência...</div>
            <div style={{ color: '#555', fontSize: '12px' }}>Consultando STF, STJ, TST e TJs</div>
          </div>
        )}
          ...
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
              <div style={{ fontSize: '13px', color: '#888' }}>
                <span style={{ color: '#C9A84C', fontWeight: '600' }}>4 decisões</span> encontradas para "{query || 'cláusula de não-concorrência validade'}"
              </div>
                ...
                <div key={i} className="card" style={{ padding: '20px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '12px' }}>
                    <div style={{ display: 'flex', gap: '8px', alignItems: 'center', flexWrap: 'wrap' }}>
                      ...
                      <span style={{ fontSize: '13px', color: '#888' }}>{r.tipo} {r.numero}</span>
                      <span style={{ fontSize: '12px', color: '#555' }}>· {r.turma}</span>
                    </div>
                    <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                      <div style={{
                        ...
                        <span style={{ fontSize: '10px', color: '#C9A84C', fontWeight: '600' }}>{r.relevancia}% relevante</span>
                      </div>
                    </div>
                  </div>

                  <div style={{ marginBottom: '8px' }}>
                    <span style={{ fontSize: '12px', color: '#555' }}>Rel. {r.relator} · {r.data}</span>
                  </div>
        ...
        {!searched && !loading && (
          <div style={{ textAlign: 'center', padding: '80px 0', color: '#444' }}>
            ...
            <div style={{ fontSize: '15px', fontWeight: '500', marginBottom: '8px', color: '#555' }}>Pesquise em linguagem natural</div>
            ...
            <div style={{ display: 'flex', gap: '8px', justifyContent: 'center', flexWrap: 'wrap' }}>
              {["Não-concorrência prazo máximo", "Rescisão indireta TST", "Dano moral dispensa discriminatória", "Juros contratuais abusivos"].map(s => (
                <button key={s} onClick={() => { setQuery(s); }} style={{
                  ...
                  padding: '8px 14px', fontSize: '12px', color: '#666', cursor: 'pointer'
                }}>{s}</button>
              ))}
            </div>
          </div>
